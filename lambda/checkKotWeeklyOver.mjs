...
export async function checkKotWeeklyOver(loginId, loginPassword){
    try{
        ...
        for (const tr of await trList) {
            ...
            // 残業時間
            const tmpOverDt = (await tdList[35].textContent());
            const overDt = Number(tmpOverDt.trim());
            // 割増残業
            const tmpAddOverDt = (await tdList[36].textContent());
            const addOverDt = Number(tmpAddOverDt.trim());
            // 深夜所定外
            const tmpExtraNightDt = (await tdList[38].textContent());
            const extraNightDt = Number(tmpExtraNightDt.trim());
            // 深夜残業
            const tmpOverNightDt = (await tdList[39].textContent());
            const overNightDt = Number(tmpOverNightDt.trim());
            // 割増深夜残業
            const tmpAddOverNightDt = (await tdList[40].textContent());
            const addOverNightDt = Number(tmpAddOverNightDt.trim());
            // 法定休日所定外
            const tmpExtraBankHolidayDt = (await tdList[42].textContent());
            const extraBankHolidayDt = Number(tmpExtraBankHolidayDt.trim());
            // 法定休日残業
            const tmpBankHolidayOverDt = (await tdList[43].textContent());
            const bankHolidayOverDt = Number(tmpBankHolidayOverDt.trim());
            // 法定休日深夜所定外
            const tmpExtraBankHolidayNightDt = (await tdList[45].textContent());
            const extraBankHolidayNightDt = Number(tmpExtraBankHolidayNightDt.trim());
            // 法定休日深夜残業
            const tmpExtraBankHolidayOverNightDt = (await tdList[46].textContent());
            const extraBankHolidayoverNightDt = Number(tmpExtraBankHolidayOverNightDt.trim());
            // 法定外休日所定外
            const tmpExtraNonstatutoryHolidayDt = (await tdList[48].textContent());
            const extraNonstatutoryHolidayDt = Number(tmpExtraNonstatutoryHolidayDt.trim());
            // 法定外休日残業
            const tmpNonstatutoryHolidayOverDt = (await tdList[49].textContent());
            const nonstatutoryHolidayOverDt = Number(tmpNonstatutoryHolidayOverDt.trim());
            // 法定外休日深夜所定外
            const tmpExtraNonstatutoryHolidayNightDt = (await tdList[51].textContent());
            const extraNonstatutoryHolidayNightDt = Number(tmpExtraNonstatutoryHolidayNightDt.trim());
            // 法定外休日深夜残業
            const tmpExtraNonstatutoryHolidayOverNightDt = (await tdList[52].textContent());
