 ...
 * 値が1月(01)の場合、12を返却する
 */
export function converteLastMonth(thisMonth: string): string {
    if(thisMonth === '01'){
        return '12';
    }
 ...
 * 去年の値を取得する 
 */
export function converteLastYear(thisYear: string): string {
    return String(Number(thisYear) - 1);
}

// 土日祝日を省くための関数
export function getNextBusinessDay(date: Date, daysToAdd: number) {
...
function getNthWeekdayOfMonth(year: number, month: number, nth: number, weekday: number) {
