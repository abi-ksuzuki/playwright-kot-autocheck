...
export async function getTotalOverDt(tdList){
    ...
    const name = tmpName.trim();
    // 残業時間
    const tmpOverDt = (await tdList[33].textContent()) as string;
    const overDt = Number(tmpOverDt.trim());
    // 割増残業
    const tmpAddOverDt = (await tdList[34].textContent()) as string;
    const addOverDt = Number(tmpAddOverDt.trim());
    // 深夜所定外
    const tmpExtraNightDt = (await tdList[36].textContent()) as string;
    const extraNightDt = Number(tmpExtraNightDt.trim());
    // 深夜残業
    const tmpOverNightDt = (await tdList[37].textContent()) as string;
    const overNightDt = Number(tmpOverNightDt.trim());
    // 割増深夜残業
    const tmpAddOverNightDt = (await tdList[38].textContent()) as string;
    const addOverNightDt = Number(tmpAddOverNightDt.trim());
    // 法定休日所定外
    const tmpExtraBankHolidayDt = (await tdList[40].textContent()) as string;
    const extraBankHolidayDt = Number(tmpExtraBankHolidayDt.trim());
    // 法定休日残業
    const tmpBankHolidayOverDt = (await tdList[41].textContent()) as string;
    const bankHolidayOverDt = Number(tmpBankHolidayOverDt.trim());
    // 法定休日深夜所定外
    const tmpExtraBankHolidayNightDt = (await tdList[43].textContent()) as string;
    const extraBankHolidayNightDt = Number(tmpExtraBankHolidayNightDt.trim());
    // 法定休日深夜残業
    const tmpExtraBankHolidayOverNightDt = (await tdList[44].textContent()) as string;
    const extraBankHolidayoverNightDt = Number(tmpExtraBankHolidayOverNightDt.trim());
    // 法定外休日所定外
    const tmpExtraNonstatutoryHolidayDt = (await tdList[46].textContent()) as string;
    const extraNonstatutoryHolidayDt = Number(tmpExtraNonstatutoryHolidayDt.trim());
    // 法定外休日残業
    const tmpNonstatutoryHolidayOverDt = (await tdList[47].textContent()) as string;
    const nonstatutoryHolidayOverDt = Number(tmpNonstatutoryHolidayOverDt.trim());
    // 法定外休日深夜所定外
    const tmpExtraNonstatutoryHolidayNightDt = (await tdList[49].textContent()) as string;
    const extraNonstatutoryHolidayNightDt = Number(tmpExtraNonstatutoryHolidayNightDt.trim());
    // 法定外休日深夜残業
    const tmpExtraNonstatutoryHolidayOverNightDt = (await tdList[50].textContent()) as string;
