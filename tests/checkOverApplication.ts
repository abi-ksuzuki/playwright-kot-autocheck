...
export async function checkOverApplication() {
  try {
    const client = new WebClient(process.env.SLACK_ATTACHMENTS_TOKEN as string);
  
    // チャンネルの履歴を取得
    const result = await client.conversations.history({
      channel: process.env.ATTACHMENTS_SLACK_CHANNEL_ID as string,
    });
...
export async function attachmentsCsv(response) {
  const rows: Record<string, string>[] = []; // CSVの内容を保存する配列
